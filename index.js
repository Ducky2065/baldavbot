...
function createBot() {
  ...
  bot.once('spawn', () => {
    ...
    // Execute commands to join Skyblock and warp after restart
    setTimeout(() => {
      bot.chat('/skyblock');
      setTimeout(() => {
        bot.chat('/is warp Maxim_2065');
      }, 2000);
    }, 5000);
  });
  ...
  bot.on('chat', (username, message) => {
    ...
    // Detect server restart announcement
    if (message.includes('server restart')) {
      console.log('\x1b[31m[AfkBot] Server restart detected. Preparing to reconnect...\x1b[0m');
    }
  });
  ...
  bot.on('death', () => {
    console.log(
      `\x1b[33m[AfkBot] Bot has died and was respawned ${bot.entity.position}`,
      '\x1b[0m'
    );
  });
